...
def isPropFloat(numstr):
# Check if a string is a float between 0 and 1
    try:
        num = float(numstr);
    except:
        return False;
...
def endProg(globs, final_msg=False):
# A nice way to end the program.
    ...
    if globs['warnings'] != 0:
        ...
        printWrite(globs['logfilename'], globs['log-v'], f"#\n# pare finished with {str(globs['warnings'])} {warn_str}. Be sure to check {warn_str_2} out in the log.\n#");
